...
macro_rules! define_ap_register {
    ...
    => {
        ...
        paste::paste! {
            mod [<impl_ $name:snake>] {
                use $crate::architecture::arm::ap::RegisterT;
                $(use $crate::architecture::arm::ap::v1::RegAddr as RegAddrV1;
                impl RegisterT<RegAddrV1> for super::$name {
                    const ADDRESS: RegAddrV1 = RegAddrV1($address_v1);
                    const NAME: &'static str = stringify!($name);
                })?
                $(use $crate::architecture::arm::ap::v2::RegAddr as RegAddrV2;
                impl RegisterT<RegAddrV2> for super::$name {
                    const ADDRESS: RegAddrV2 = RegAddrV2($address_v2);
                    const NAME: &'static str = stringify!($name);
                })?
            }
        }
