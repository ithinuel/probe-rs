...
use super::{
    memory::{
        registers::{self, BaseAddrFormat, BASE, BASE2, DRW, TAR, TAR2},
        DataSize, MemoryAp,
    },
    ApRegAddressT, GenericAp,
};
    ...
    ApRegAccess<BASE> + ApRegAccess<BASE2> + ApRegAccess<TAR> + ApRegAccess<TAR2> + ApRegAccess<DRW>
{
    ...
    fn read_data<I: ApAccess>(
        ...
        .map_err(AccessPortError::old_register_read_error::<DRW, _>)
    ...
    fn write_data<I: ApAccess>(
        ...
        .map_err(AccessPortError::old_register_write_error::<DRW, _>)
...
impl AccessPortError {
    ...
    /// Constructs a [`AccessPortError::RegisterRead`] from just the source error and the register type.
    pub fn old_register_read_error<R: Register, E: std::error::Error + Send + Sync + 'static>(
        source: E,
    ) -> Self {
        AccessPortError::RegisterRead {
    ...
    /// Constructs a [`AccessPortError::RegisterWrite`] from just the source error and the register type.
    pub fn old_register_write_error<R: Register, E: std::error::Error + Send + Sync + 'static>(
        source: E,
    ) -> Self {
        AccessPortError::RegisterWrite {
            address: R::ADDRESS.into(),
